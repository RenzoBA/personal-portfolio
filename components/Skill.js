...
const Skill = ({ leftSide, skill: { title, progress, image } }) => {
  return (
    <motion.div
      ...
      whileInView={{ x: 0 }}
      viewport={{ once: true }}
      className="group relative flex justify-center cursor-pointer bg-gray-700 shadow-[#202020] shadow-lg rounded-full border-none"
    >
      <img
        src={urlFor(image).url()}
        className="border border-gray-500 rounded-full object-cover w-20 h-20 md:w-24 md:h-24 filter group-hover:blur-[2px] group-hover:scale-125 group-hover:opacity-40 transition duration-300 ease-in-out p-1"
      />
